...
import { Item, ItemForm } from './restoration';
...
export class ApiService {
  ...
  private log(message: string) {
    this.messageService.add(`Реставрации: ${message}`);
  };
  ...
  editToItem({comment, employeeForm, endDate, exhibitForm, startDate}: ItemForm) {
    const employee = Object.create({});
    const exhibit = Object.create({});

    employee.id = employeeForm;
    exhibit.id = exhibitForm;
    return {comment, employee, endDate, exhibit, startDate};
  }

  editToForm({comment, employee, endDate, exhibit, startDate}: Item) {
    const employeeForm = employee.id;
    const exhibitForm = exhibit.id;

    return {comment, employeeForm, endDate, exhibitForm, startDate};
  }
}
